...
CREATE TABLE `room` (
  `off_id` decimal(4,0) NOT NULL,
  `com_name` varchar(20) DEFAULT NULL,
  `off_situation` bit(1) NOT NULL DEFAULT b'0',
  `off_money` float NOT NULL DEFAULT '15000',
  PRIMARY KEY (`off_id`),
  KEY `FK_tenant` (`com_name`),
  CONSTRAINT `FK_tenant` FOREIGN KEY (`com_name`) REFERENCES `company_info` (`com_name`) ON DELETE CASCADE ON UPDATE CASCADE
